...
async function runScan(options = {}) {
  ...
  try {
    ...
    const tech = state.techFilters.join(",");
    const data = await api(`/api/scan?market=${state.market}&volumePair=${encodeURIComponent(state.volumePair)}&tech=${encodeURIComponent(tech)}${refresh}&_=${Date.now()}`, 180000);
    state.scan = data;
...
function renderScan(data) {
  ...
  const [shortDays, longDays] = (data.volumePair || state.volumePair).split("-");
  const selectedTech = data.techFilters || state.techFilters;
  el("criteriaText").textContent = `篩出 ${shortDays} 日均量大於 ${longDays} 日均量，且至少連續 2 個交易日成立${selectedTech.length ? `，另套用 ${selectedTech.length} 項技術條件` : ""}的強勢股。`;
  el("activeTechCount").textContent = selectedTech.length ? `已套用 ${selectedTech.length} 項` : "未套用";
...
async function loadDetail(item) {
  ...
  el("detailSub").textContent = `AI分數 ${item.ai.score}｜成交 ${lots(item.volume)} 張｜${item.summary.volumePairLabel} ${fmt(item.summary.volumeMomentum)} 倍｜主力集中度(推估) ${item.summary.mainForceConcentration}`;
...
function renderReasons(item) {
  ...
  const metrics = [
    ...
    ["主力籌碼", `集中度(推估) ${item.summary.mainForceConcentration}｜400張 ${fmt(latest.large400Change, 0)} 張，1000張 ${fmt(latest.large1000Change, 0)} 張`]
  ];
...
el("volumePair").addEventListener("change", (event) => {
  state.volumePair = event.target.value;
  runScan();
});
document.querySelectorAll(".tech-option input").forEach((input) => {
  input.addEventListener("change", () => {
    state.techFilters = [...document.querySelectorAll(".tech-option input:checked")].map((item) => item.dataset.tech);
    el("activeTechCount").textContent = state.techFilters.length ? `已套用 ${state.techFilters.length} 項` : "未套用";
    runScan();
  });
});
